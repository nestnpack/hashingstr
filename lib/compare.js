...
const md4 = require('./compare/md4.js');
const md5 = require('./compare/md5.js');
const mdc2 = require('./compare/mdc2.js');
...
const ripemd = require('./compare/ripemd.js');
...
function compare(type, value, hashedValue) {
  ...
  //list algorithm
  const list = ['md4', 'md5', 'mdc2', 'sha1', 'sha2-224', 'sha2-256', 'sha2-384', 'sha2-512', 'sha3-224', 'sha3-256', 'sha3-384', 'sha3-512', 'whirlpool', 'ripemd', 'ripemd160', 'shake128', 'shake256', 'sm3', 'blake2b512', 'blake2s256'];
  
  //if type is a string
  if(typeof type === 'string') {
    //md
    if(type === 'md4') {
      return md4(value, hashedValue);
    }
    ...
    //ripemd
    if(type === 'ripemd') {
      return ripemd(value, hashedValue);
    }
